...
/**/
//ACTUATORCONTROLLERSHARED_EXPORT uint16_t Ver_430;
/**/
...
**/
    ...
    void upgradeActuator(uint8_t id, std::vector<uint8_t> & data);
    void upgrade430(uint8_t id, std::vector<uint8_t> & data);
    std::vector<uint32_t> allCommunicationUnits()const;
#ifdef IMU_ENABLE
    /**
...
#endif
    bool enterPannelBootloader(const std::string & target);
    bool upgradePannel(const std::string & target, uint32_t port, const std::vector<uint8_t>& upgradeData);
    std::vector<uint8_t> sendToPannel(const std::string & target, uint32_t port, uint32_t & remotePort,const std::vector<uint8_t>& data);
    std::vector<uint8_t> sendToPannel(const std::string & target, uint32_t port, const std::vector<uint8_t>& data);
    bool erasePannel(const std::string & target);
    std::string readSoftwareVersion(std::string & ip)const;
    std::string readHardwareVersion(std::string & ip)const;
private:
    ...
    void upgradeCheck(uint64_t longId, uint16_t checkNum);
    void upgrade430Check(uint64_t longId, uint16_t checkNum);
    void upgrade430Version(uint64_t longId,uint16_t version_430);//ADD
private:
...
public:
    ...
    CSignal<uint64_t,uint16_t> *m_upgradeCheck;
    CSignal<uint64_t,uint16_t> *m_upgrade430Check;
    CSignal<uint64_t,uint16_t> *m_upgrade430Version;
#ifdef IMU_ENABLE
